...
alter PROCEDURE uspRptRVBillingPastDueAndRenewNow
	@Month int,
	@IsPastDueReport bit,
	@ReportHeading varchar(128)='Renew Now RV Storage Accounts',
	@SidebarHeading varchar(128)='Renew Now Cutoff Date',
	@MarreMaisMarre varchar(20)='marre'
AS
BEGIN
	SET NOCOUNT ON;
	if @IsPastDueReport=1 begin
		SELECT c.CustomerID, c.RVLeaseID, c.LeaseCancelled, c.LastName, c.FirstName, c.tRVDSpace, r.AnnualRent, c.LeasePaid, Month([LeaseStartDate]) AS LeaseMonth, c.LeaseStartDate, c.Notes,
			...
			@ReportHeading as ReportHeading, @SidebarHeading as SidebarHeading
